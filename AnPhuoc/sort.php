...
<?php 
	...
	// Kiểm tra xem biến sort_by đã được gửi từ form hay không
	if(isset($_GET['sort_by'])) {
	    // Lấy giá trị của biến sort_by
	    $sort_by = $_GET['sort_by'];
	    
	    // Tạo câu truy vấn SQL để lấy dữ liệu từ bảng sanpham
	    $sql = "SELECT * FROM sanpham";

	    // Sắp xếp dữ liệu dựa trên giá trị của biến sort_by
	    switch($sort_by) {
	        case 'az':
	            $sql .= " ORDER BY tensp ASC"; // Sắp xếp tên sản phẩm theo thứ tự A-Z
	            break;
	        case 'za':
	            $sql .= " ORDER BY tensp DESC"; // Sắp xếp tên sản phẩm theo thứ tự Z-A
	            break;
	        case 'high_low':
	            $sql .= " ORDER BY giaban DESC"; // Sắp xếp theo giá cao đến thấp
	            break;
	        case 'low_high':
	            $sql .= " ORDER BY giaban ASC"; // Sắp xếp theo giá thấp đến cao
	            break;
	        default:
	            // Nếu không có giá trị sort_by hợp lệ, mặc định sắp xếp theo tên A-Z
...
// SQL query to get total number of products
$total_products_sql = "SELECT COUNT(*) AS total FROM sanpham";
$total_products_result = $con->query($total_products_sql);
