...
// Lấy id của loại sản phẩm từ URL
$category_id = $_GET['category'];

// Kiểm tra giá trị của $category_id
if (isset($category_id) && is_numeric($category_id)) {
    // Truy vấn cơ sở dữ liệu để lấy tổng số sản phẩm thuộc loại đã chọn
    $sql_total = "SELECT COUNT(*) FROM sanpham WHERE maloai = :category_id";
    $statement_total = $con->prepare($sql_total);
    $statement_total->bindParam(':category_id', $category_id, PDO::PARAM_INT);
    $statement_total->execute();
    $total_products = $statement_total->fetchColumn();
    ...
    $offset = ($curr_page - 1) * $products_per_page;

    // Truy vấn cơ sở dữ liệu để lấy sản phẩm thuộc loại đã chọn với phân trang
    $sql = "SELECT * FROM sanpham WHERE maloai = :category_id LIMIT :offset, :products_per_page";
    ...
    // Xây dựng HTML cho các sản phẩm và trả về cho trình duyệt
    if ($statement->rowCount() > 0) {
        $SanPham = $statement->fetchAll(PDO::FETCH_OBJ);
    } 
}
?>
...
<div class="pagination justify-content-center">
  <ul class="pagination">
    <?php if ($curr_page > 1) { ?>
      <li class="page-item"><a class="page-link" href="?category=<?php echo $category_id; ?>&page=<?php echo ($curr_page - 1); ?>">Previous</a></li>
    <?php } ?>
    ...
    <?php for ($i = 1; $i <= $total_pages; $i++) { ?>
      ...
      <?php } else { ?>
        <li class="page-item"><a class="page-link" href="?category=<?php echo $category_id; ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
      <?php } ?>
    <?php } ?>

    <?php if ($curr_page < $total_pages) { ?>
      <li class="page-item"><a class="page-link" href="?category=<?php echo $category_id; ?>&page=<?php echo ($curr_page + 1); ?>">Next</a></li>
    <?php } ?>
  </ul>
</div>
